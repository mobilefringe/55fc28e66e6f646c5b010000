...
setEndPoint('//faubourgboisfranc.mallmaverick.com/api/v4/faubourg/all.json');
...
function renderHomeHours(container, template, collection){
    ...
    $.each( item_list , function( key, val ) {
        if (val.open_time && val.close_time && (val.is_closed == false || val.is_closed == null)){
            ...
            val. hours = open_time.format("h:mm A") + " - " + close_time.format("h:mm A")
        } else {
            val.hours = "Closed Today";
        }
	...
	$.reject({  
		...
		browserInfo: { // Settings for which browsers to display  
			...
			firefox: {  
				...
				url: '//www.mozilla.com/firefox/'  
			},  
			safari: {  
				text: 'Safari',  
				url: '//www.apple.com/safari/download/'  
			},  
			opera: {  
				text: 'Opera',  
				url: '//www.opera.com/download/'  
			},  
			msie: {  
				text: 'Internet Explorer',  
				url: '//www.microsoft.com/windows/Internet-explorer/'  
			}  
